...
public class DynaShopAPI {
    ...
    public static void OpenShopGUI(Player player, String shopName, int page)
    {
        ...
        {
            infoLore.add("§7 - "+ DynamicShop.ccShop.get().getInt(shopName+".Options.SalesTax") + "%");
        }
        else
        {
            infoLore.add("§7 - "+ DynamicShop.plugin.getConfig().getInt("SalesTax") + "%");
        }
        ...
        if(GetShopBalance(shopName) >= 0)
        {
            ...
            {
                infoLore.add("§7 - " + d + "Points");
            }
            else
            {
                infoLore.add("§7 - " + DynamicShop.getEconomy().format(d));
            }
        }
        else
        {
        ...
        for (String s:DynamicShop.ccShop.get().getConfigurationSection(shopName).getKeys(false))
        {
            try
            {
                ...
                if(DynamicShop.ccShop.get().contains(shopName+"."+s+".value"))
                {
                    ...
                    if(DynamicShop.ccShop.get().contains(shopName+".Options.flag.jobpoint"))
                    {
                        lore.add(DynamicShop.ccLang.get().getString("PRICE") + value);
                    }
                    else
                    {
                        lore.add(DynamicShop.ccLang.get().getString("PRICE") + value);
                    ...
                    if(!DynamicShop.ccShop.get().getBoolean(shopName+".Options.hideStock")) lore.add(DynamicShop.ccLang.get().getString("STOCK") + stockStr);
            ...
            {
                if(!s.equalsIgnoreCase("Options")) DynamicShop.console.sendMessage(DynamicShop.dsPrefix_server+"ERR.OpenShopGui/Failed to create itemstack. incomplete data. check yml.");
...
//                }
            ...
            ItemStack taxToggleBtn =  CreateItemStack(Material.IRON_INGOT,null,
                    ...
                    new ArrayList<>(Arrays.asList(
                            ...
                            "§e"+DynamicShop.ccLang.get().getString("CLICK")+": " +
                                    DynamicShop.ccLang.get().getString("TAX.USE_GLOBAL").replace("{tax}",tax+"")
                    )),1);
    ...
    public static void OpenItemTradeInven(Player player, String shopName, String tradeIdx)
    {
        ...
        {
            priceStr = DynamicShop.ccLang.get().getString("PRICE");
        }
        else
        {
            priceStr = DynamicShop.ccLang.get().getString("PRICE");
        ...
        ItemStack sellBtn = DynaShopAPI.CreateItemStack(Material.GREEN_STAINED_GLASS,null,
                sellStr,sellLore,1);
        ItemStack buyBtn = DynaShopAPI.CreateItemStack(Material.RED_STAINED_GLASS,null,
                buyStr,buyLore,1);
        ...
        if(optionS.contains("flag.jobpoint"))
        {
            if(balStr.contains(DynamicShop.ccLang.get().getString("SHOP_BAL_INF")))
            {
                moneyLore.add("§f" + balStr);
            }
            else
            {
                moneyLore.add("§f" + balStr + "Points");
            }
        }
        else
        {
            ...
            if(d>=0)
            {
                moneyLore.add("§f" + DynamicShop.getEconomy().format(d));
            }
            else
            {
                moneyLore.add("§f" + balStr);
...
//                            }
    ...
    public static double CalcTotalCost(String shopName, String idx, int amount)
    {
        ...
        if(amount < 0)
        {
            if(DynamicShop.ccShop.get().contains(shopName+".Options.SalesTax"))
            {
                total = total - ((total / 100) * DynamicShop.ccShop.get().getInt(shopName+".Options.SalesTax"));
            }
            else
            {
                total = total - ((total / 100) * DynamicShop.plugin.getConfig().getDouble("SalesTax"));
            }
        }
